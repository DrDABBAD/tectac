...
WebAssembly is a compact bytecode format optimized for fast download and maximum execution speed.  DOTNET code runs inside the context of WebAssembly. "a .NET" and blazor is the single page app that sits on the the dotnet.
...
The the blazor  app is executed directly on the browser UI thread.

The deployment of the app can happen in two ways.

* [Hosted deployment with ASP.NET Core](https://docs.microsoft.com/en-us/aspnet/core/blazor/host-and-deploy/webassembly?view=aspnetcore-5.0#hosted-deployment-with-aspnet-core) ,where the Blazor app is served by an ASP.NET Core app.

* [Standalone deployment](https://docs.microsoft.com/en-us/aspnet/core/blazor/host-and-deploy/webassembly?view=aspnetcore-5.0#standalone-deployment) The Blazor app is placed on a static hosting web server or service such as Azure App Service.
